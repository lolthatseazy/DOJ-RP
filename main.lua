if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("DOJ:RP Yield is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "DOJ:RP Yield is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "DOJ:RP Yield FE v" .. currentVersion
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- DOJ:RP Yield Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the DOJ:RP Yield support server.'}
...
CMDs[#CMDs + 1] = {NAME = 'whizz [plr or all]', DESC = 'Plays bullet sound'}
CMDs[#CMDs + 1] = {NAME = 'smokecar', DESC = 'Makes your car go smokey smokey'}
CMDs[#CMDs + 1] = {NAME = 'deploy', DESC = 'Instantly respawns you, can be used to bypass out of jail'}
...
addcmd('deploy', {}, function()
	game:GetService("ReplicatedStorage").GuiEvents.deploy:FireServer()
end)

addcmd('smokecar', {}, function()
    notify("Warning", "USE BY HOLDING F, THIS CANT BE DISABLED")
    ...
    local function updateSmoke()
        while isHolding do
            workspace.Game_Items.Player_Vehicles[game.Players.LocalPlayer.Name.."'s Car"].rauch:FireServer(table.unpack({
                [1] = "UpdateSmoke",
                [2] = math.huge,
                [3] = 0,
            }))
...
addcmd('whizz', {}, function(args, speaker)
    if args == "all" then
        for _, v in pairs(game:GetPlayers()) do
            game.ReplicatedStorage:FindFirstChild("ACS_Engine").Eventos.Whizz:FireServer(v)
        end
        return
    end
    ...
    for _, v in pairs(players:GetPlayers()) do
        if string.find(v.Name:lower(), "^" .. args:lower()) or string.find(v.DisplayName:lower(), "^" .. args:lower()) then
            target = v
            break
        end
    end
    ...
    if target then
        game.ReplicatedStorage:FindFirstChild("ACS_Engine").Eventos.Whizz:FireServer(target)
    else
        notify("Warning", "Player not found")
    end
end)
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, DOJ:RP Yield may have issues or features that are not functioning correctly.") end
end)
